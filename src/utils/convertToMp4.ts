...
export async function convertWebmToMp4(blob: Blob): Promise<Blob> {
  ...
  const presets = [
    ['-i', 'input.webm', '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '28', '-c:a', 'aac', '-movflags', '+faststart', 'output.mp4'],
    ['-i', 'input.webm', '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '28', '-an', '-movflags', '+faststart', 'output.mp4'],
